...
from transformers import AutoModelForCausalLM, AutoModelForImageTextToText, AutoTokenizer, BitsAndBytesConfig, PreTrainedTokenizer, PreTrainedModel
import torch
...
# Models that are VL (vision-language) architectures and need AutoModelForImageTextToText instead of AutoModelForCausalLM
vl_model_names = {
    "Ministral-3-14B-Instruct-2512-BF16",  # HF config has model_type="mistral3" (VL arch)
    "gemma-3-1b-it",   # HF config has model_type="gemma3" (Gemma3ForConditionalGeneration)
    "gemma-3-4b-it",
    "gemma-3-12b-it",
    "gemma-3-27b-it",
}
...
class ModelInfo(TypedDict):
    """This class (a dict) has the attributes: model, tokenizer"""
    model: PreTrainedModel
    tokenizer: PreTrainedTokenizer

def get_basemodel_loadstring(engine, handle_model_paths=True):
    if handle_model_paths:
        engine = engine.split("/")[-1]
    else:
        if "/" in engine:
            raise ValueError("Engine cannot be a path when handle_model_paths=False")
    if "+" in engine:
...
def load_model(engine, checkpoints_dir = None, device_map = "auto", full_32_precision=False, \
               brainfloat=False, verbose=False, load_in_8bit=False, load_in_4bit=False, lora_model=False, trust_remote_code=True, manual_precision=True, is_trainable=False) -> ModelInfo:
    """Can handle many types of models."""

    loadstring = get_basemodel_loadstring(engine)
    base_model_name = loadstring.split("/")[-1]
    auto_cls = AutoModelForImageTextToText if base_model_name in vl_model_names else AutoModelForCausalLM
    ...
    if load_in_8bit:
        ...
        quantization_config = BitsAndBytesConfig(
            load_in_4bit=True,
            bnb_4bit_quant_type="nf4",  # or "fp4" if you want
    ...
    if engine.endswith("quantized_model"):  # FULLLY SAVED MODEL
        ...
        # model.to(device)
    elif engine.endswith("lora_model") or lora_model or "lora_model" in engine:  
        tokenizer = AutoTokenizer.from_pretrained(loadstring, trust_remote_code=trust_remote_code)
